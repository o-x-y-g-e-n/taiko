...
const _closeBrowser = async () => {
  ...
  if (_client) {
    logEvent('Removing listeners');
    await _client.removeAllListeners();
    logEvent('Removed listeners');
    logEvent('Closing page');
    if (page) await page.close();
    logEvent('Closed page');
    logEvent('Closing client');
    await _client.close();
    logEvent('Closed client');
    _client = null;
  }
  logEvent('Trying to close browser');
  if (chromeProcess && !chromeProcess.killed) {
    const waitForChromeToClose = new Promise(fulfill => {
      chromeProcess.once('exit', () => {
        logEvent('Chrome process exited');
        fulfill();
      });
    });
    chromeProcess.kill('SIGTERM');
    logEvent('SIGTERM sent');
    await waitForChromeToClose;
    if (temporaryUserDataDir) {
      try {
        fs.removeSync(temporaryUserDataDir);
      } catch (e) {}
      logEvent('temporary user data dir removed');
    }
  }
  logEvent('Closed browser');
  chromeProcess = null;
};
...
// TODO: Deprecated. To be removed after removing inputField and textField.
 ...
 * (await textField('username').elements())[0].value() # same as above
 *
 * $('.class').text()
